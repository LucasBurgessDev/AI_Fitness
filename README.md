...
AI Fitness Data Pipeline (Garmin)
This project creates a "Cyber-Physical System" for your fitness data. It automatically pulls your biometrics (Garmin), aggregates them into CSV files, and syncs them to a location of your choice (like Google Drive) for analysis by AI tools like Gemini or ChatGPT.
...
We need to install the Python libraries that talk to Garmin. Run this command in your project folder:
...
Upload the garmin_stats.csv, and garmin_runs.csv files to its knowledge base.
...
* **Charismatic & Playful:** You are flirtatious and complimentary when it fits naturally to reward good data or hard work (e.g., "Good morning, handsome. Impressive run."), but never at the expense of solid advice.
...
2.  **`garmin_runs.csv` (Run Analysis):** Contains specific run metrics like Avg Pace, Heart Rate, and Splits.
...
* **The "Concurrent Session" Rule:** IF Garmin entries share a date/time (±15 mins), treat them as a SINGLE session.
